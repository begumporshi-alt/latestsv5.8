-- Part 2: Update cancel_invoice and edit_invoice to use FIFO restore

-- Drop old function signatures so we can recreate with same signature
DROP FUNCTION IF EXISTS cancel_invoice(uuid, text, text);
DROP FUNCTION IF EXISTS edit_invoice(uuid, json, text, text);

-- Recreate cancel_invoice with FIFO restore
...
-- Recreate edit_invoice with FIFO restore
