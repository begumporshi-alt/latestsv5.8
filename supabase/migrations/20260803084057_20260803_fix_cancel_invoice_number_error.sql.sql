/*
# Fix cancel_invoice Number() Error and Reversal Payment Date

## Problem
- The cancel_invoice function uses `Number(v_payment.amount)` which is JavaScript syntax, not PL/pgSQL.
- PostgreSQL has no `Number()` function, causing "function number(numeric) does not exist" error.
- The reversal payment INSERT also uses CURRENT_DATE instead of the invoice date.

## Fix
- Replace `Number(v_payment.amount)` with `v_payment.amount` (already numeric).
- Replace CURRENT_DATE with COALESCE(v_invoice.invoice_date, CURRENT_DATE) in the reversal payment INSERT.
*/
