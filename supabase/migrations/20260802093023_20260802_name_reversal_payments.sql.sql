/*
# Name NULL payment_for records as 'reversal_payment'

These are system-generated reversal payments created by the cancel_invoice
and edit_invoice database functions. They reverse original customer payments
when an invoice is cancelled or edited. They are not customer-initiated payments.
*/

-- Update NULL payment_for to 'reversal_payment'
UPDATE payments SET payment_for = 'reversal_payment' WHERE payment_for IS NULL;

-- Update cancel_invoice function to set payment_for = 'reversal_payment' on reversal payments
-- (The function creates REV-PAY-* payments without payment_for)
CREATE OR REPLACE FUNCTION cancel_invoice(p_invoice_id uuid, p_reason text DEFAULT NULL, p_cancelled_by text DEFAULT NULL)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  FOR v_payment IN SELECT * FROM payments WHERE reference_id = p_invoice_id AND reference_type = 'invoice' AND is_reversed = false LOOP
    ...
    INSERT INTO payments (
      ...
      'REV-PAY-' || nextval('payment_number_seq'),
      'received',
      'invoice_cancel',
      p_invoice_id,
      v_invoice.customer_id,
  ...
  IF v_total_payments > 0 AND v_ar_account IS NOT NULL THEN
    -- Find the cash/bank account from the payment journal entries
    PERFORM post_journal_entry(
